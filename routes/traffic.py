    ...
    try:
        ...
        for tl in world.get_actors().filter("traffic.traffic_light*"):
            ...
            if dist <= radius:
                ...
                lights.append({
                    "id": tl.id, "state": state, "distance": round(dist, 1),
...
def traffic_light_set():
    try:
        ...
        if not actor: return jsonify({"success": False, "error": "Actor not found"})
...
def traffic_light_freeze_all():
    try:
        ...
        world = get_world()
        for tl in world.get_actors().filter("traffic.traffic_light*"):
            if state_key in TL_STATE_MAP: tl.set_state(TL_STATE_MAP[state_key])
            tl.freeze(freeze)
        return jsonify({"success": True})
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
