...
async function loadEmergencyBlueprints() {
  ...
  if (data.success && data.blueprints.length > 0) {
    ...
    addLog(`Emergency blueprints: ${data.blueprints.length} found`, "info");
  } else {
    sel.innerHTML = `<option value="">⚠ None found — using random</option>`;
    addLog("No emergency vehicle blueprints found in library", "warn");
  }
}
...
async function spawnEmergency() {
  ...
  const label = bp || "random emergency";
  toast(`🚨 Spawning ${label}…`, "info");
  addLog(`Spawning emergency vehicle: ${label}`, "info");
  const data = await api("/spawn/emergency", "POST", {
    blueprint: bp,
    autopilot: autopilot,
    at_spectator: atSpectator,
  });
  if (data.success) {
    ...
    addLog("Emergency spawn error: " + data.error, "err");
  }
}
...
async function fixAllVehicles() {
  toast("🔧 Applying strict TM rules to all vehicles…", "info");
  addLog("Fixing all vehicle TM rules (anti-circular driving)…", "info");
  const data = await api("/tm/fix_all", "POST");
  if (data.success) {
    toast(`✅ Fixed ${data.fixed}/${data.total} vehicle(s) — lane discipline applied`, "ok", 4000);
    addLog(`TM fix applied: ${data.fixed}/${data.total} vehicles — auto_lane_change=OFF, keep_right=100%`, "ok");
  } else {
    toast("❌ Fix failed: " + data.error, "err", 5000);
    addLog("TM fix_all error: " + data.error, "err");
  }
}
